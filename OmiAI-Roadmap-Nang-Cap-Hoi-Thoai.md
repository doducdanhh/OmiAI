...
## PHẦN 8 — HƯỚNG DẪN CẤP MÃ NGUỒN CHO TỪNG MODULE

Đây là khung sườn (không phải implementation đầy đủ — đúng văn hoá "honest scaffolding" của chính dự án) để AI coding agent bám vào, dùng đúng tên kiểu dữ liệu thật đã xác nhận có trong repo.

### 8.1. `crates/omiai-io/src/router.rs` (mới — trái tim của Slice 7)

```rust
use omiai_core::logic_engine::Formula;
use omiai_core::inference::ProofResult;
use omiai_core::prover::{TheoremProver, ProofReport};
use omiai_knowledge::graph::KnowledgeGraph;
use omiai_probabilistic::bayesian::BayesianNetwork;
use omiai_causal::dag::CausalDag;

/// Kết quả suy luận hợp nhất — MỌI pillar trả lời đều quy về đây, để lớp
/// diễn đạt (Slice 9) chỉ cần xử lý một kiểu dữ liệu duy nhất.
#[derive(Debug, Clone)]
pub enum ReasoningResult {
    Proved { formula: Formula, report: ProofReport },
    Disproved { formula: Formula, report: ProofReport },
    Probabilistic { query: String, probability: f64, evidence: Vec<(String, bool)> },
    Causal { cause: String, effect: String, is_causal: bool, adjustment_set: Vec<String> },
    KnowledgePath { from: String, to: String, path: Vec<String> },
    WorldQuery { summary: String, raw_agent_count: usize, raw_vocabulary_size: usize },
    Unverified { note: String },
}

pub struct DialogueRouter {
    prover: TheoremProver,
    bayes_nets: Vec<(String, BayesianNetwork)>,  // tên miền → mạng đã nạp
    causal_dags: Vec<(String, CausalDag)>,
}

impl DialogueRouter {
    /// Thứ tự thử LÀ một quyết định thiết kế, không tuỳ tiện:
    /// Bayesian trước (trả lời được "bao nhiêu %" mà prover nhị phân
    /// không làm được) → Causal (trả lời "tại sao") → logic hình thức
    /// (mặc định, hành vi đã có) → đồ thị tri thức (phương án cuối trước
    /// khi nhận thua).
    pub fn route(
        &self,
        ...
        // (1) và (2): xem Slice 8 — cần NLU nhận diện được ý định
        // "hỏi xác suất" / "hỏi nhân quả" trước khi router có gì để thử.
        // Khung hàm để AI coding agent điền khi Slice 8 xong:
        // fn try_bayesian(&self, ...) -> Option<ReasoningResult>
        // fn try_causal(&self, ...) -> Option<ReasoningResult>

        // (3) mặc định — TÁI DÙNG đúng logic đã có trong chat.rs::answer_query,
        // không viết lại từ đầu:
        if let Some(q) = query {
            let report = self.prover.prove_timed(premises, q);
            if matches!(report.result, ProofResult::Proved { .. }) {
                return ReasoningResult::Proved { formula: q.clone(), report };
            }
            let negated = Formula::Not(Box::new(q.clone()));
            let neg_report = self.prover.prove_timed(premises, &negated);
            if matches!(neg_report.result, ProofResult::Proved { .. }) {
                return ReasoningResult::Disproved { formula: q.clone(), report: neg_report };
            }
            // (4) phương án cuối trước khi báo "chưa đủ dữ kiện"
            if let Some(path) = graph.query_path(&format!("{q:?}"), "") {
                // (placeholder minh hoạ — thực thi thật cần trích subject/object
                // từ Formula, không phải Debug string; xem ghi chú bên dưới)
                let _ = path;
            }
        }
        ReasoningResult::Unverified {
            note: "chưa đủ dữ kiện trong logic, xác suất, nhân quả, hay đồ thị tri thức".into(),
        }
    }
}
```

**Ghi chú bắt buộc đọc trước khi implement:** đoạn `graph.query_path(&format!("{q:?}"), "")` ở trên là chỗ đánh dấu rõ ràng "đây là placeholder, KHÔNG copy y nguyên" — `query_path` cần hai id nút thật (kiểu `&str`), không phải chuỗi Debug của cả Formula. AI coding agent phải viết một hàm trích subject/predicate từ `Formula::Atom` trước, test riêng hàm trích xuất đó, rồi mới nối vào `query_path`. Đây chính là lằn ranh giữa "khung sườn trung thực" (được phép) và "code giả vờ chạy" (không được phép) mà toàn bộ tài liệu này yêu cầu.

### 8.2. `docs/format-spec/bundle-v1.md` + `crates/omiai-export/src/lib.rs`

Xem đặc tả đầy đủ ở Phần 6. Khung hàm tối thiểu cho `omiai-export`:

```rust
pub struct BundleManifest { /* đúng schema JSON ở Phần 6.2 */ }

pub enum ExportError {
    SourceCheckpointNotFound(std::path::PathBuf),
    NoDataForDeclaredCapability(&'static str),
    IoError(std::io::Error),
}

/// Đọc một checkpoint (thư mục, đã có `omiai-checkpoint`), cắt tỉa, đóng gói
/// thành MỘT file `.omiai` (tar → zstd). KHÔNG được đặt `capabilities.X = true`
/// nếu không có payload thật cho X — vi phạm điều này là vi phạm hợp đồng nạp
/// (load contract) ở Phần 6.3.
pub fn export_bundle(
    checkpoint_dir: &std::path::Path,
    output_path: &std::path::Path,
    options: ExportOptions,
) -> Result<BundleManifest, ExportError> {
    todo!("Slice 11 — xem Phần 6 và Phần 7 Slice 11 để biết thứ tự thao tác")
}
```

(Đây là nơi DUY NHẤT trong toàn tài liệu cố ý dùng `todo!()` — vì Phần 8 chỉ là khung chữ ký hàm, không phải code hoàn chỉnh; agent thực thi sẽ thay `todo!()` bằng implementation thật, và **không được để `todo!()` tồn tại trong bất kỳ commit nào được đánh dấu "xong"**, đúng quy tắc đã nêu.)

### 8.3. `crates/omiai-runtime/src/lib.rs`
...
pub struct OmiaiModel { /* nạp từ bundle: prover state, graph, bayes nets, causal dags, reservoir, ... */ }

impl OmiaiModel {
    pub fn load(bundle_path: &std::path::Path) -> Result<Self, RuntimeError> {
        todo!("giải nén tar+zstd, đọc manifest.json, kiểm BLAKE3 từng file, dựng từng pillar theo đúng cờ capabilities")
    }

    /// Điểm vào DUY NHẤT mà omiai-serve / omiai-cli / FFI / WASM gọi tới.
    pub fn step(&mut self, input: &str) -> InferOutput {
        todo!("gọi DialogueRouter::route rồi lớp diễn đạt (Slice 9/10)")
    }
}
...
pub struct InferOutput {
    pub text: String,
    pub grounded: bool,           // xem nguyên tắc bất biến ở Slice 10
    pub reasoning: Option<String>, // biểu diễn debug/giải thích của ReasoningResult
}
```

Ba đích biên dịch (native / `cdylib` / `wasm32-wasi` / `wasm32-unknown-unknown`) khai báo trong `Cargo.toml`:

```toml
[lib]
crate-type = ["rlib", "cdylib"]
# wasm32-unknown-unknown build riêng, dùng wasm-bindgen, KHÔNG chung crate-type
# với cdylib native trong cùng một lần build — biên dịch tách theo target,
# xem `cargo build --target wasm32-unknown-unknown` vs build mặc định.
```

### 8.4. `crates/omiai-serve/src/main.rs`

```rust
use axum::{routing::post, Json, Router};

#[derive(serde::Deserialize)]
struct InferRequest { input: String, session_id: Option<String> }

async fn infer(
    axum::extract::State(model): axum::extract::State<std::sync::Arc<tokio::sync::Mutex<omiai_runtime::OmiaiModel>>>,
    Json(req): Json<InferRequest>,
) -> Json<omiai_runtime::InferOutput> {
    let mut model = model.lock().await;
    Json(model.step(&req.input))
}

#[tokio::main]
async fn main() {
    let bundle_path = std::env::args().nth(1).expect("cách dùng: omiai-serve <đường-dẫn-model.omiai>");
    let model = omiai_runtime::OmiaiModel::load(std::path::Path::new(&bundle_path))
        .expect("không nạp được bundle — xem RuntimeError để biết lý do chính xác");
    let state = std::sync::Arc::new(tokio::sync::Mutex::new(model));
...
### 8.5. Mẫu `docs/adr/0008-optional-local-llm-surface-layer.md` (Slice 10)

```markdown
# ADR-0008: Lớp diễn đạt ngôn ngữ cục bộ tuỳ chọn (pillar thứ 9)

Status: proposed → accepted khi Slice 10 có test grounding xanh

## Bối cảnh
Bảy/tám trụ cột hiện có giải quyết suy luận (symbolic + probabilistic + causal
+ reservoir) nhưng không giải quyết được sự trôi chảy ngôn ngữ mở — đây là
giới hạn tài nguyên vật lý (dữ liệu + compute huấn luyện), không phải giới
hạn kỹ thuật có thể vượt qua bằng cách viết thêm luật tay.

## Quyết định
Cho phép một mô hình ngôn ngữ nhỏ, mã nguồn mở, giấy phép permissive, chạy
100% cục bộ (không gọi API ngoài), CHỈ đóng vai trò diễn đạt lại một
`ReasoningResult` đã được lõi symbolic xác minh — không bao giờ được quyết
định sự thật mới.

## Hệ quả
- Bundle `.omiai` lớn hơn đáng kể khi bật pillar này (vài trăm MB–vài GB).
- Cần theo dõi giấy phép mô hình nguồn (ghi trong `language_model_info`,
  Phần 6.2) — không phân phối lại bundle công khai nếu giấy phép không cho phép.
- `capabilities.language_model = false` theo mặc định trong mọi bundle xuất ra,
  trừ khi người vận hành bật rõ ràng bằng cờ khi export.

## Ràng buộc thực thi bắt buộc (không chỉ là tài liệu — phải có test)
Xem bài test "grounding" ở Slice 10, Phần 7: mọi con số/tên riêng trong câu
trả lời của mô hình ngôn ngữ phải khớp với `ReasoningResult` đã đưa vào
prompt; không khớp = test fail.
```

---

## PHẦN 9 — NGÂN SÁCH PHẦN CỨNG & HIỆU NĂNG (8 GB RAM / i7-7700K)

| Thành phần | RAM ước tính | Ghi chú |
|---|---:|---|
| Hệ điều hành + runtime Rust cơ bản | ~500 MB – 1 GB | Cố định, không tránh được |
| `omiai-world` (CA grid 1024×1024, 1 byte/cell) | ~1 MB/lớp grid | Rẻ — README đã tính đúng |
| `omiai-world` agents + registry (tuỳ số lượng agent) | vài chục–vài trăm MB | Đo thật bằng benchmark trước khi cấu hình số agent lớn |
| `omiai-neuro` Reservoir (≤ 2000 units, RLS cần ma trận P kích thước n²) | ~32 MB tại n=2000 (2000² × 8 byte) | README đã tự giới hạn đúng vì lý do này — **không tăng size reservoir mà không tính lại** |
| `omiai-knowledge` / `omiai-probabilistic` / `omiai-causal` (đồ thị, mạng Bayes, DAG cỡ vừa) | vài chục MB | Nhẹ so với các thành phần khác |
| **Mô hình ngôn ngữ cục bộ (Slice 10, tuỳ chọn)** | **500 MB – 2,5 GB tuỳ mô hình/lượng tử hoá** | **Thành phần lớn nhất — chọn nhỏ nhất chấp nhận được trước** |
| Biên an toàn khuyến nghị | ≥ 1,5 GB dư | Tránh OOM khi hệ điều hành cấp phát đột biến |

**Quy tắc quyết định cho Slice 10:** cộng tổng các dòng trên; nếu vượt quá ~6,5 GB (chừa 1,5 GB an toàn trên máy 8 GB), giảm kích thước mô hình ngôn ngữ trước, không giảm reservoir/world (chúng rẻ và là giá trị cốt lõi). Luôn đo bằng công cụ thật (`/usr/bin/time -v`, hoặc `heaptrack`) sau khi implement — không suy đoán từ bảng trên khi ra quyết định cuối.

---

## PHẦN 10 — RỦI RO, GIỚI HẠN, VÀ NHỮNG GÌ KHÔNG NÊN TUYÊN BỐ

| ĐỪNG nói | NÊN nói thay vào đó |
|---|---|
| "AI này có ý thức / hiểu ngôn ngữ như con người" | "AI này suy luận có thể kiểm chứng bằng logic hình thức, xác suất, và nhân quả; trong phạm vi tri thức đã dạy, độ chính xác là X% trên bộ test Y" |
| "Tự tạo ra trí tuệ từ Unicode" | "Dùng Unicode đúng cách để xử lý văn bản (tokenizer), đặt suy luận ở lớp symbolic/probabilistic đã được kiểm chứng" |
| "Mạng nơ-ron này hiểu ngôn ngữ" (nói về reservoir) | "Reservoir dự đoán động lực học/đa dạng hoá diễn đạt; hiểu ngôn ngữ nằm ở semantic parser + knowledge graph" |
| "Chatbot trôi chảy như GPT, tự huấn luyện 100%" | "Diễn đạt trôi chảy (nếu bật Slice 10) nhờ một mô hình mã nguồn mở đã huấn luyện sẵn, chỉ dùng để đặt câu cho đúng sự thật đã xác minh — ghi rõ nguồn, giấy phép" |
| Bất kỳ con số hiệu năng nào không có benchmark `criterion` đứng sau | Luôn kèm số + lệnh để người khác tự chạy lại kiểm chứng |

**Rủi ro kỹ thuật cụ thể cần canh trong lúc thực thi:**
- **Vòng phụ thuộc Cargo** khi `omiai-io` bắt đầu phụ thuộc thật vào nhiều pillar (Slice 7) — chạy `cargo tree -e no-dev` để phát hiện sớm.
- **Giấy phép mô hình ngôn ngữ** (Slice 10) — một số mô hình mạnh dùng giấy phép cộng đồng có điều kiện (ví dụ giới hạn theo số người dùng hàng tháng); đọc kỹ trước khi đóng gói vào bundle sẽ phân phối công khai.
- **Semantic parser tiến hoá (Slice 8) có thể overfit lên tập 300–800 câu mẫu** — bắt buộc tách tập train/test riêng biệt, báo cáo độ chính xác trên tập test chưa từng thấy, không chỉ tập train.
- **Reservoir với input là văn bản** (nếu mở rộng ngoài vai trò đa dạng hoá diễn đạt) dễ bị hiểu lầm là "đang học ngôn ngữ" trong khi thực chất chỉ học một ánh xạ tuyến tính trên trạng thái động lực học — luôn ghi rõ trong docstring vai trò thật của mỗi lần dùng reservoir, tránh chính codebase tự gây hiểu lầm về sau.

---

## PHẦN 11 — CHECKLIST THỰC THI TUẦN TỰ

```
[ ] 0. Chạy `cargo test --workspace` trong môi trường thật — xác nhận con số
       README tự báo cáo trước khi tin bất kỳ điều gì trong tài liệu này.
[ ] 1. Slice 7 — chuyển 5 pillar từ dev-dependencies sang dependencies thật
       trong omiai-io; viết DialogueRouter + ReasoningResult; integration
       test chứng minh mỗi pillar được GỌI THẬT từ ChatEngine.
[ ] 2. Slice 8 — gộp lexicon vào knowledge::graph; ngữ pháp phụ thuộc tối
       giản; (tuỳ chọn nhưng khuyến nghị mạnh) tiến hoá luật ánh xạ bằng
       evolution::formula_gp trên 300–800 cặp câu↔Formula tự tạo.
[ ] 3. Slice 9 — realize_* đọc từ ReasoningResult; đa dạng hoá bằng
       reservoir; thêm ParseIntent::AskWorld truy vấn omiai-world read-only.
[ ] 4. QUYẾT ĐỊNH: có bật Slice 10 (LLM cục bộ) hay dừng ở kiến trúc thuần
       symbolic? Cả hai đều là một hệ thống "xong" và trung thực.
[ ] 5. (nếu bật) Slice 10 — viết ADR-0008; chọn llama-cpp-2 hoặc candle;
       chọn mô hình theo ngân sách RAM (Phần 9); prompt ràng buộc chặt;
       bài test grounding bắt buộc.
[ ] 6. Slice 11 — omiai-export: đặc tả bundle-v1.md (Phần 6) → code →
       round-trip test.
[ ] 7. Slice 12 — omiai-runtime: load/step; 3 đích biên dịch.
[ ] 8. Slice 13 — omiai-serve: axum /infer.
[ ] 9. Slice 14 — omiai-cli: train/resume/export/bench/chat/serve.
[ ] 10. Slice 15 — tests/ đầu-cuối ở root; examples/world_demo.rs;
        examples/communication_demo.rs; cập nhật README status table
        trung thực theo đúng những gì vừa làm xong.
```
...
- H. Jaeger, *The "echo state" approach to analysing and training recurrent neural networks* (2001) — nền tảng lý thuyết của `omiai-neuro::reservoir`.
- D. Sussillo & L. F. Abbott, *Generating Coherent Patterns of Activity from Chaotic Neural Networks* (FORCE learning, 2009) — trích dẫn đúng trong code hiện có.
- J. Pearl, *Causality: Models, Reasoning, and Inference* — nền tảng của `omiai-causal` (do-calculus, back-door criterion, counterfactual).
- K. Friston, *The free-energy principle: a unified brain theory?* (2010) — nền tảng của Active Inference trong `omiai-meta::self_improvement`.
- D. Lewis, *Convention: A Philosophical Study* (1969) — nguồn gốc lý thuyết trò chơi tín hiệu (signaling game) dùng trong `omiai-world::communication`.
- Kho `candle` (Hugging Face) — framework ML thuần Rust, hỗ trợ CPU/WASM, `candle-transformers` cho kiến trúc Llama/Qwen/Phi.
- Kho `llama-cpp-2` (`utilityai/llama-cpp-rs`) — binding Rust cho `llama.cpp`, đọc GGUF, tối ưu CPU/AVX2.
- Định dạng GGUF (hệ sinh thái `llama.cpp`/Hugging Face) — định dạng lượng tử hoá mô hình phổ biến nhất cho suy luận CPU cục bộ tính đến thời điểm viết tài liệu.

---

*Hết tài liệu roadmap. Xem file thứ hai đi kèm — bản "chỉ thị thực thi" (agent prompt) — để có phiên bản cô đọng, mệnh lệnh hoá, tối ưu cho một AI coding agent bám theo từng bước, cùng đặc tả `.omiai` ở mức chính xác cao nhất.*
