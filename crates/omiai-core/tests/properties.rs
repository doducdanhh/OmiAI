...
use omiai_core::logic_engine::{self, Formula, Literal, Term};
use omiai_core::prover::TheoremProver;
use omiai_core::substitution::Substitution;
use omiai_core::unification::{UnificationError, unify};
...
proptest! {
    ...
    fn occurs_check_blocks_infinite_term(x in "[a-z]") {
        ...
        let r = unify(&t1, &t2);
        let is_occurs = matches!(&r, Err(UnificationError::OccursCheckFailed { .. }));
...
proptest! {
    ...
    fn prover_modus_ponens(
        ...
        prop_assert!(
            matches!(r, omiai_core::inference::ProofResult::Proved { .. }),
            "expected Proved, got {:?}",
            r
        );
    }
