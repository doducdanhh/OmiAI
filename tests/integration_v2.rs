...
//!
//! - [`omiai::probabilistic::junction_tree`] — Junction Tree exact inference.
//! - [`omiai::probabilistic::hmc`] — Hamiltonian Monte Carlo.
//! - [`omiai::core::higher_order_unification`] — Huet HO unification.
//! - [`omiai::knowledge::abduction`] — Consistency-based abduction.
//! - [`omiai::knowledge::discocat`] — Distributional compositional categorial.
//! - [`omiai::causal::icp`] — Invariant Causal Prediction.
//! - [`omiai::meta::autopoiesis`] — Self-improvement loop.
//! - [`omiai::probabilistic::puct_mcts`] — PUCT-style MCTS.
//!
//! Run with: `cargo test --release --test integration_v2`

use std::collections::HashMap;

use omiai::causal::icp::{IcpSample, icp};
use omiai::core::higher_order_unification::{Term, Type};
use omiai::knowledge::abduction::{abduce, best_explanation};
use omiai::knowledge::discocat::{cosine, parse_transitive, toy_lexicon};
use omiai::knowledge::graph::{Concept, KnowledgeGraph};
use omiai::meta::autopoiesis::{AutopoieticLoop, MarkovBlanket, WorldState};
use omiai::probabilistic::bayesian::{BayesianNetwork, Cpt};
use omiai::probabilistic::hmc::{HmcSampler, IsotropicNormal, StandardNormal_};
use omiai::probabilistic::junction_tree::JunctionTree;
use omiai::probabilistic::mcts::GameState;
use omiai::probabilistic::puct_mcts::PuctMcts;
...
fn huet_finds_eta_solution() {
    ...
    let sols = omiai::core::higher_order_unification::unify_terms(&[(lhs, a)], 4);
...
fn abduce_wet_grass_returns_minimal_explanations() {
    let rains = omiai::core::logic_engine::Formula::prop("rain");
    let sprinkler = omiai::core::logic_engine::Formula::prop("sprinkler");
    let wet = omiai::core::logic_engine::Formula::prop("wet");
    let rule1 =
        omiai::core::logic_engine::Formula::Implies(Box::new(rains.clone()), Box::new(wet.clone()));
    let rule2 = omiai::core::logic_engine::Formula::Implies(
